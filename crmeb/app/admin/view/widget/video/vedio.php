...
<head>
    ...
    <script src="{__MODULE_PATH}widget/vedio-upload.js"></script>
</head>
<style>
                                ...
                                <div class="layui-tree-set layui-tree-setHide">
                                    <div class="layui-tree-entry">
                                        <div class="layui-tree-main" @click="OpenTree({child:[],id:0})">
                                            ...
                                            <span class="layui-tree-txt" :class="pid == 0 ? 'on' : '' ">全部视频</span>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="layui-card text-center">
                    <div class="layui-card-header">
                        <div class="layadmin-homepage-pad-ver" style="text-align: left">
                            <div class="layui-btn-group" id="container">
<!--                                <button type="button" class="layui-btn layui-btn-normal layui-btn-sm"  @click="addCategory">添加分类</button>-->
                                <button type="button" class="layui-btn layui-btn-normal layui-btn-sm" ref="upload">上传视频</button>
                    ...
                    <div class="layui-card-body clearfix image-box" style="padding: 10px;height: 360px;z-index:10;">
                        <div class="layui-col-md2 layui-col-xs2 layui-col-sm2 image" v-for="(item,index) in imageList">
                            ...
                            <div class="layui-img-box"  :class="item.isSelect ? 'on': '' ">
                                <video :src="item.att_dir" width="80;" v-if="small == 1" @click="changImage(item)"></video>
                                <video :src="item.att_dir" width="80;" v-else @click="changImage(item)"></video>
                            </div>
                        </div>
        ...
        methods:{
            ...
            useImages:function(){
                ...
                if(parentinputname == 'editor'){
                    if(parentinputname == 'editor' && this.selectImages.length > 1) {
                        return layList.msg('最多只能选择1张');
                    }
                    var src = this.selectImages[0];
                    ...
                    if(parent.$f){
                        ...
                        if(parent.$vm){
                            ...
                            if(rule.maxLength !== undefined){
                                ...
                                parent.changeVEDIO(parentinputname,this.selectImages);
                                var index = parent.layer.getFrameIndex(window.name);
                                parent.layer.close(index);
                                return;
                            }
                        ...
                        parent.changeVEDIO(this.selectImages[0]);
                        var index = parent.layer.getFrameIndex(window.name);
                        parent.layer.close(index);
                    }
                }
...
//                console.log(this.selectImagesIDS)
            ...
            getImageList:function(){
                ...
                layList.baseGet(this.U({a:'get_image_list',q:{pid:20,page:this.page,limit:this.limit}}),function (res) {
                    that.loading = false;
...
//                if(!this.searchTitle) return layList.msg('请输入搜索内容！');
            ...
            U:function(opt){
                ...
                return layList.U({m:'admin',c:"widget.vedio",a:opt.a || '',q:opt.q || {},p:opt.q || {}});
            },
            //获取分类
            ...
            uploal:function () {
                ...
                this.uploadInst=layList.upload.render({
                    ...
                    url: this.U({a:'TempKeys'}),
                    accept: 'video', //视频
                    auto: true,
                    choose:function(obj){
                        obj.preview(function(index, file){
                            that.$set(that,'vedio',file);
                        });
                    },
                    done:function(res){
                        if(res.type=="OSS"){
                            var client = new OSS.Wrapper({
                                region: res.region,
                                accessKeyId: res.id,
                                accessKeySecret: res.key,
                                bucket: res.bucket,
                            });
                        }else if(res.type=='COS'){
                            var client = new COS({
                                SecretId: res.id,
                                SecretKey: res.key,
                            })
                        }else if(res.type=='QINIU'){
                            var client = res.uploadToken;
                            console.log(client);
                        }
                        UserFactory(res.type,that.vedio,client,res,function(src,file,is){
                            console.log(src,file.size,file.type);
                            layList.basePost(that.U({a:'save'}),{src:src,size:file.size,type:file.type,is:is},function (res) {
                                layList.msg(res.msg);
                                that.getImageList();
                            },function (res) {
                                layList.msg(res.msg);
