    ...
    public function getSystemInfo()
    {
        ...
        $info["environment"] = [
            ...
            [
                "name" => "MySql版本",
                "require" => "5.6-8.0",
                "value" => Db::query("SELECT VERSION()")[0]["VERSION()"],
            ],
            [
                "name" => "MySqli",
                "require" => "开启",
            ...
            [
                "name" => "Session",
                "require" => "开启",
                "value" => function_exists("session_start"),
            ],
            [
                "name" => "Safe_Mode",
                "require" => "开启",
            ...
            [
                "name" => "GD",
                "require" => "开启",
                "value" => !empty($gd_info["GD Version"]),
            ],
            [
                "name" => "Curl",
                "require" => "开启",
            ...
            [
                "name" => "Bcmath",
                "require" => "开启",
                "value" => function_exists("bcadd"),
            ],
            [
                "name" => "Upload",
                "require" => "开启",
                "value" => (bool) ini_get("file_uploads"),
            ],
        ];
        ...
        $info["permissions"] = [
            [
                ...
                "require" => "读写",
                "value" =>
                    is_readable(root_path("backup")) &&
                    is_writable(root_path("backup")),
            ],
            [
                "name" => "public",
                "require" => "读写",
                "value" =>
                    is_readable(root_path("public")) &&
                    is_writable(root_path("public")),
            ],
            [
                "name" => "runtime",
                "require" => "读写",
                "value" =>
                    is_readable(root_path("runtime")) &&
                    is_writable(root_path("runtime")),
            ],
            [
                "name" => ".env",
                "require" => "读写",
                "value" =>
                    is_readable(root_path() . ".env") &&
                    is_writable(root_path() . ".env"),
            ],
            [
                "name" => ".version",
                "require" => "读写",
                "value" =>
                    is_readable(root_path() . ".version") &&
                    is_writable(root_path() . ".version"),
            ],
            [
                "name" => ".constant",
                "require" => "读写",
                "value" =>
                    is_readable(root_path() . ".constant") &&
                    is_writable(root_path() . ".constant"),
            ],
        ];
        if (function_exists("exec")) {
            $workermanOutput = $timerOutput = $queueOutput = [];
            exec(
                "ps aux | grep 'php think workerman' | grep -v grep",
                $workermanOutput,
            );
            exec(
                "ps aux | grep 'php think timer' | grep -v grep",
                $timerOutput,
            );
            exec(
                "ps aux | grep 'php think queue' | grep -v grep",
                $queueOutput,
            );
            $info["process"] = [
                [
                    "name" => "长链接",
                    "require" => "开启",
                    "value" => count($workermanOutput) > 0,
                ],
                [
                    "name" => "定时任务",
                    "require" => "开启",
                    "value" => count($timerOutput) > 0,
                ],
                [
                    "name" => "消息队列",
                    "require" => "开启",
                    "value" => count($queueOutput) > 0,
                ],
            ];
        } else {
            $info["process"] = [
                [
                    ...
                    "require" => "开启",
                    "value" => file_exists(
                        root_path("runtime") . "workerman.pid",
                    ),
                ],
                [
                    "name" => "定时任务",
                    "require" => "开启",
                    "value" => file_exists(root_path("runtime") . ".timer"),
                ],
                [
                    "name" => "消息队列",
                    "require" => "开启",
                    "value" => file_exists(root_path("runtime") . ".queue"),
                ],
            ];
        }
