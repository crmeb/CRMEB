...
parser.prototype.onOpenTag = function(selfClose) {
	...
	if (node.name == 'embed') {
		...
		if (src.includes('.mp4') || src.includes('.3gp') || src.includes('.m3u8') || (attrs.type || '').includes('video'))
			node.name = 'video';
		else if (src.includes('.mp3') || src.includes('.wav') || src.includes('.aac') || src.includes('.m4a') || (attrs.type ||
...
parser.prototype.popNode = function() {
	...
	if (node.name == 'a' || node.name == 'ad' // #ifdef H5 || APP-PLUS
		...
		if (!attrs.poster && !attrs.autoplay) attrs.poster =
		...
		if (this.options.pauseVideo) str +=
