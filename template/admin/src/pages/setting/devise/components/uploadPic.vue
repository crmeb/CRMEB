<template>
  <div class="hot_imgs">
        ...
        <div class="item" v-for="(item, index) in listData" :key="index">
          ...
          <div class="info">
            <div class="info-item" v-if="item.hasOwnProperty('name')">
              <span>{{
                type == 1 ? "管理名称：" : type == 2 ? "广告名称" : "服务名称："
              }}</span>
              <div class="input-box">
                <Input
                  v-model="item.name"
                  :placeholder="type == 2 ? '请输入名称' : '服务中心'"
                  :maxlength="4"
                />
              </div>
            </div>
    ...
    <template v-if="listData">
      <div
        class="add-btn"
        v-if="(type != 1 && type != 2) || (type == 2 && listData.length < 5)"
      >
        <Button
          type="primary"
          ghost
  ...
  methods: {
    ...
    addBox() {
      if (this.listData.length == 0) {
        ...
        let obj = JSON.parse(
          JSON.stringify(this.listData[this.listData.length - 1])
        );
        obj.name = "";
        obj.pic = "";
        obj.url = "";
