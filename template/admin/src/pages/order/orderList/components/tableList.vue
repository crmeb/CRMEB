    ...
    <div>
        ...
        >
            ...
            <template slot-scope="{ row, index }" slot="action">
                ...
                <a @click="sendOrder(row)" v-if='row._status === 2 && row.shipping_type === 1 && !row.pinkStatus'>发送货</a>
                <a @click="sendOrder(row)" v-if='row._status === 2 && row.shipping_type === 1 && row.pinkStatus === 2'>发送货</a>
                ...
                <Divider type="vertical" v-if='row._status === 2 && row.shipping_type === 1 && row.pinkStatus === 2'/>
                <Divider type="vertical" v-if='row._status === 1 || (row._status === 2 && !row.pinkStatus) || row._status === 4 || (row.shipping_type == 2 && row.status == 0  && row.paid == 1 && row.refund_status === 0)'/>
                <template>
