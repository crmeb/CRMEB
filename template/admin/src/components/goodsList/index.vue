                ...
                <Col v-bind="grid" v-if="!type && diy">
                    <FormItem label="商品类型：" label-for="pid">
                        <Select v-model="goodType" style="width:200px" clearable @on-change="userSearchs">
                            <Option v-for="item in goodList" :value="item.activeValue" :key="item.activeValue">{{ item.title }}
                            </Option>
                        </Select>
                    </FormItem>
                </Col>
                <Col v-bind="grid">
                    <FormItem label="商品搜索：" label-for="store_name">
                        <Input search enter-button placeholder="请输入商品名称,关键字,编号" v-model="formValidate.store_name"
                               style="width:200px" @on-search="userSearchs"/>
                    </FormItem>
                </Col>
            </Row>
        </Form>
